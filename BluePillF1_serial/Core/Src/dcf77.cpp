...
#include "dcf77.h"
...
#if ( defined(STM32F1) || defined(STM32F4) ) && defined(STM32_CORE_VERSION)  // $siST_V0_02/03 block
  // https://github.com/stm32duino/STM32Examples/blob/main/examples/Peripherals/HardwareTimer/Timebase_callback/Timebase_callback.ino
	...
	TIM_TypeDef *Instance = TIM1;   // for bluepill chose TIM1 to TIM4 but check for clashes with your application
...
namespace Internal { namespace Debug {
    void debug_helper(char data) { dcf_putn(data == 0? 'S': data == 1? '?': data - 2 + '0', 0); }
...
namespace BCD {
    void print(const bcd_t value) {
        dcf_putn(value.val >> 4 & 0xF, HEX);
        dcf_putn(value.val >> 0 & 0xF, HEX);
    }
...
namespace Internal {  // DCF77_Flag_Decoder
    ...
    void DCF77_Flag_Decoder::debug() {
        dcf_puts(F("Backup Antenna, TZ change, TZ, Leap scheduled, Date parity: "));
        dcf_putn(abnormal_transmitter_operation, BIN);
        dcf_putc(',');
        dcf_putn(timezone_change_scheduled, DEC);
        dcf_putc(',');
        dcf_putn(uses_summertime, DEC);
        dcf_putc(',');
        dcf_putn(leap_second_scheduled, DEC);
        dcf_putc(',');
        dcf_putnln(date_parity, DEC);
    }
}
...
namespace Internal {  // DCF77_Second_Decoder
    ...
    void DCF77_Second_Decoder::debug() {
        ...
        if (prev_tick == this->tick) {
            ...
            dcf_puts(F("second: "));
            dcf_putn(get_time_value(), DEC);
            dcf_puts(F(" Sync mark index "));
            Binning::Decoder<DCF77_Second_template_parameters::decoder_t::data_type,
                             DCF77_Second_template_parameters::decoder_t::number_of_bins>::debug();
            dcf_puts(F("Prediction Match: "));
            dcf_putnln(prediction_match, DEC);
            dcf_putnl();
        }
    }
}
...
namespace Internal {  // DCF77_Encoder
    ...
    void DCF77_Encoder::debug() const {
        ...
        dcf_putn(leap_second_scheduled, 10);
        dcf_putc(',');
        dcf_putn(timezone_change_scheduled, 10);
        dcf_putc(' ');
    }
    ...
    void DCF77_Encoder::debug(const uint16_t cycles) const {
        ...
        dcf_puts(F("M ?????????????? RAZZA S mmmmMMMP hhhhHHP ddddDD www mmmmM yyyyYYYYP S"));
        for (uint16_t second = 0; second < cycles; ++second) {
            switch (local_clock.second) {
                case  0: dcf_putnl(); break;
                case  1: case 15: case 20: case 21: case 29:
                case 36: case 42: case 45: case 50: case 59: dcf_putc(' ');
            }
...
namespace DCF77_Clock {
    ...
    void print(Clock::time_t time) {
        ...
        BCD::print(time.day);
        dcf_putc(' ');
        dcf_putn(time.weekday.val & 0xF, HEX);
        dcf_putc(' ');
        BCD::print(time.hour);
        dcf_putc(':');
        ...
        dcf_putn(time.timezone_change_scheduled? '*': '.', 10);
        dcf_putn(time.leap_second_scheduled    ? 'L': '.', 10);
    }
...
namespace Internal {  // DCF77_Frequency_Control
    ...
    void DCF77_Frequency_Control::debug() {
        using namespace Debug;
        dcf_putsln(F("confirmed_precision ?? adjustment, deviation, elapsed"));
        sprintpp16m(confirmed_precision);
        dcf_puts(F(", "));
        dcf_putn(calibration_state.running? '@': '.', 10);
        dcf_putn(calibration_state.qualified? '+': '-', 10);
        dcf_putc(' ');
        ...
        dcf_putn(deviation, 10);
        dcf_puts(F(" ticks, "));

        dcf_putn(deviation_tracker.elapsed_minutes, 10);
        dcf_puts(F(" min + "));
        dcf_putn(deviation_tracker.elapsed_ticks_mod_60000, 10);
        dcf_putsln(F(" ticks mod 60000"));
    }
...
namespace Internal {
    namespace Generic_1_kHz_Generator {
        ...
        #elif ( defined(STM32F1) || defined(STM32F4) ) && defined(STM32_CORE_VERSION)
        void setup(const Clock::input_provider_t input_provider) {
            const uint32_t prescaleFactor = HAL_RCC_GetSysClockFreq() / 1000000UL;
            the_input_provider = input_provider;
            MyTim->setOverflow(1000);
            MyTim->setPrescaleFactor(prescaleFactor);
            MyTim->attachInterrupt(isr_handler);
            MyTim->resume();
        }
        const uint16_t inverse_timer_resolution = 16000;
        void isr_handler() {
        ...
        #else
        // STM32 CubeIDE / HAL puro:
        // setup() salva solo l'input provider; il timer e' configurato da CubeMX.
        // isr_handler() non viene chiamata dal bridge ma deve esistere perche'
        // e' dichiarata nel namespace.
        void setup(const Clock::input_provider_t input_provider) {
            the_input_provider = input_provider;
        }
        void isr_handler() {
