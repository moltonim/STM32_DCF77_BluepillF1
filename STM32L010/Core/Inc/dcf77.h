...
#include "Arduino.h"
#else
/* STM32 CubeIDE: sostituisce Arduino.h con il compatibility layer HAL */
#include "dcf77_hal_adapter.h"    // ← aggiunto
#endif
...
//    #endif
    ...
    namespace Binning {
        ...
        struct bins_t {
            ...
            void debug() {
                dcf_putn(this->get_time_value().val, HEX);
                dcf_puts(F(" Tick: "));
                dcf_putn(this->tick, 10);
                dcf_puts(F(" Quality: "));
                dcf_putn(this->signal_max, DEC);
                dcf_putc('-');
                dcf_putn(this->noise_max, DEC);
                dcf_puts(F(" Max Index: "));
                dcf_putn(this->max_index, DEC);
                dcf_puts(F(" Quality Factor: "));
                dcf_putnln(this->get_quality_factor(), DEC);
                dcf_putc('>');
            }

            void dump() {
                for (index_t index = 0; index < number_of_bins-1; ++index) {
                    dcf_putn(data[index], 10);
                    dcf_putc(',');
                }
                dcf_putnln(data[number_of_bins-1], 10);
            }
        };
        ...
        struct Decoder : bins_t<data_type, data_type, number_of_bins> {
            ...
            void debug() {
                ...
                for (index_t index = 0; index < number_of_bins; ++index) {
                    dcf_putc((index == this->signal_max_index ||
                            index == ((this->signal_max_index+1) % number_of_bins))
                           ? '|': ',');
                    dcf_putn(this->data[index], HEX);
                }
                dcf_putnl();
            }
    ...
    struct DCF77_Demodulator : Binning::Convoluter<uint16_t, Clock_Controller::Configuration::phase_lock_resolution> {
        ...
        void debug_verbose() {
            ...
            dcf_putsln(F("max_index, max, index, integral"));
            ...
            for (index_t bin = 0; bin < bin_count; ++bin) {
                ...
                dcf_putn(max_index, 10);
                dcf_puts(F(", "));
                dcf_putn(max, 10);
                dcf_puts(F(", "));
                dcf_putn(bin, 10);
                dcf_puts(F(", "));
                dcf_putnln(integral, 10);
            }
            ...
            dcf_puts(F("noise_index, noise_max: "));
            dcf_putn(noise_index, 10);
            dcf_puts(F(", "));
            dcf_putnln(noise_max, 10);
        }
    };
    ...
    struct DCF77_Local_Clock {
        ...
        void debug() {
            dcf_puts(F("Clock state: "));
            switch (clock_state) {
                case Clock::useless:  dcf_putsln(F("useless"));  break;
                case Clock::dirty:    dcf_putsln(F("dirty"));    break;
                case Clock::free:     dcf_putsln(F("free"));     break;
                case Clock::unlocked: dcf_putsln(F("unlocked")); break;
                case Clock::locked:   dcf_putsln(F("locked"));   break;
                case Clock::synced:   dcf_putsln(F("synced"));   break;
                default:              dcf_putsln(F("undefined"));
            }
            dcf_puts(F("Tick: "));
            dcf_putnln(tick, 10);
        }
...
// TODO msres: how to deal with lower vs. higher resolution, different arguments needed
    ...
    struct DCF77_Clock_Controller {
        ...
        static void debug() {
            ...
            dcf_putn(get_overall_quality_factor(), DEC);
            dcf_puts(F(" p("));
            dcf_putn(clock_quality.phase.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.phase.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.phase, DEC);

            dcf_puts(F(") s("));

            dcf_putn(clock_quality.second.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.second.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.second, DEC);

            dcf_puts(F(") m("));

            dcf_putn(clock_quality.minute.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.minute.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.minute, DEC);

            dcf_puts(F(") h("));

            dcf_putn(clock_quality.hour.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.hour.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.hour, DEC);

            dcf_puts(F(") wd("));

            dcf_putn(clock_quality.weekday.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.weekday.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.weekday, DEC);

            dcf_puts(F(") D("));

            dcf_putn(clock_quality.day.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.day.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.day, DEC);

            dcf_puts(F(") M("));

            dcf_putn(clock_quality.month.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.month.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.month, DEC);

            dcf_puts(F(") Y("));

            dcf_putn(clock_quality.year.lock_max, DEC);
            dcf_putc('-');
            dcf_putn(clock_quality.year.noise_max, DEC);
            dcf_putc(':');
            dcf_putn(clock_quality_factor.year, DEC);
            dcf_puts(F(") "));

            dcf_putn(clock_quality.uses_summertime_quality, DEC);
            dcf_putc(',');
            dcf_putn(clock_quality.timezone_change_scheduled_quality, DEC);
            dcf_putc(',');
            dcf_putn(clock_quality.leap_second_scheduled_quality, DEC);

            dcf_putc(',');
            dcf_putnln(get_prediction_match(), DEC);
        }
    };
